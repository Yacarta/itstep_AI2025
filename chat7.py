...
WIX_SITE_URL = os.getenv('ONLINE_STORE_URL')
...
titles = {
    'en': "🤖 Puro y Organico Assistant",
    'es': "🤖 Asistente de Puro y Orgánico",
    'uk': "🤖 Асистент Puro y Organico"
}

captions = {
    'en': "Your personal shopping assistant for Puro y Organico store",
    'es': "Tu asistente personal de compras para la tienda Puro y Orgánico",
    'uk': "Ваш особистий помічник з покупок для магазину Puro y Organico"
}
SHOP_INFO = {
    'delivery': {
        'cost': {
...
- Nombre del producto
- Precio
- Precio con descuento (si está disponible) discountedPrice
- URL del producto
...
def get_products_by_keyword(keyword, max_results=20, timeout=15):
    ...
    try:
        ...
        for product in products:
            # First try to get image from mainImage.url
            image_url = product.get('mainImage', {}).get('url')
            raw_description = product.get('description', 'No description available')
            clean_description = re.sub(r'</?(div|span|p)[^>]*>', '', raw_description)

            # If not found, try to extract from mainMedia (Wix format)
            if not image_url and 'mainMedia' in product:
                media_str = product['mainMedia']
                if media_str.startswith('wix:image://'):
                    # Extract the media ID part and construct URL
                    media_id_part = media_str.split('~')[0].replace('wix:image://v1/', '')
                    image_url = f"https://static.wixstatic.com/media/{media_id_part}~mv2.png"

            product_info = {
                'name': product.get('name', 'No name available'),
                'price': product.get('formattedPrice', 'Price not available'),
                'discount_price': product.get('discountedPrice', 'No'),
                'url': f"{WIX_SITE_URL}/product-page/{product.get('slug', '')}",
                'image': image_url or 'No image available',
                'description': clean_description or 'No description available'
            }
...
def format_product_response(products, language):
    """Show product cards in Streamlit with image, name, price or discount price if available, link"""
    if not products:
        st.warning({
    ...
    for product in products:
        ...
        with cols[0]:
            if product['image'] != 'No image available':
        ...
        with cols[1]:
            ...
            if product['discount_price'] != 'No discount price available':
                st.markdown(f"💲 **{product['discount_price']}**")
            else:
                st.markdown("💲 No discount price available")
...
def initialize_chatbot():
    ...
    if 'agent' not in st.session_state:
        st.session_state.agent = create_react_agent(
            model=llm,
            tools=[get_products_by_keyword, format_product_response],
        )
...
def main():
    ...
    st.title("""🤖  Puro y Organico Assistant
                🤖  Asistente de Puro y Orgánico
                🤖  Асистент Puro y Organico""")
    st.caption("""
    🛍️ Your personal shopping assistant for the Puro y Organico store  
    🛍️ Tu asistente personal de compras para la tienda Puro y Orgánico  
    🛍️ Ваш персональний помічник для покупок у магазині Puro y Organico
    """)

    # Initialize chatbot (sets st.session_state.agent and messages)
    initialize_chatbot()
    ...
    input_prompts = {
        ...
        'uk': "Чим я можу вам допомогти сьогодні?"
    }

    # Chat input
    if prompt := st.chat_input(input_prompts.get(DEFAULT_LANGUAGE, input_prompts['uk'])):
        ...
        # Replace initial system message with language-specific version
        st.session_state.messages[0] = get_system_message(lang)

        # First try to handle general questions (e.g. hours, location, etc.)
