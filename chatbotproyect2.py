...
def get_products_by_keyword(keyword, max_results=20, timeout=15):
    ...
    try:
        ...
        formatted_products = []
        for product in products:
            media_src = product.get("mediaItems", [{}])[0].get("src", "")
            image_url = "No image available"

            if media_src.startswith("wix:image://v1/"):
                # Strip prefix
                cleaned_src = media_src.replace("wix:image://v1/", "")
                # Get the media ID before any '/' or '#' or '?'
                media_id = cleaned_src.split('/')[0].split('#')[0].split('?')[0]
                image_url = f"https://static.wixstatic.com/media/{media_id}"
            elif media_src:
                image_url = media_src
            product_info = {
                'name': product.get('name', 'No name available'),
                'price': product.get('price', 'Price not available'),
                'discount': product.get('discountedPrice', 'No discount available'),

                'formatted_price': product.get('formattedPrice', 'Price not available'),
                'formatted_discount_price': product.get('formattedDiscountedPrice', 'No discount available'),
                'url': f"{WIX_SITE_URL}/product-page/{product.get('slug', '')}",
                'image': image_url,
                'short_description': product.get('description', '').split('<p>')[1].split('</p>')[0] if product.get(
                    'description') else 'No description available',
                'brand': product.get('brand', 'Brand not available'),
                'in_stock': product.get('inStock', False),
                'sku': product.get('sku', 'SKU not available')
            }
...
def format_product_response(products, language):
    ...
    if language == 'uk':
        ...
        for item in products:
            response_text += (
                f"📌 Назва: {item['name']}\n"
                f"💵 Ціна: {item['price'] + ' COP'}\n"
                f"Знижка: {item['formatted_discount_price'] + ' COP'}\n"
        ...
        for item in products:
            response_text += (
                f"📌 Nombre: {item['name']}\n"
                f"💵 Precio: {item['price'] + ' COP'}\n"
                f"Descuento: {item['formatted_discount_price'] + ' COP'}\n"
                f"🔗 Enlace: {item['url']}\n"
...
def handle_general_question(query, language):
    ...
    # Payment questions
        ...
        return SHOP_INFO['payment'][language]
...
def start_conversation():
    print("🤖 ¡Bienvenido a Puro y Orgánico! ¿Cómo puedo ayudarte hoy?")
    messages = [get_system_message(DEFAULT_LANGUAGE)]
    ...
    while True:
        ...
        messages[0] = get_system_message(lang)

        # First check if it's a general question
        general_response = handle_general_question(user_input, lang)
        if general_response:
            print(f"AI: {general_response}")
            continue

        # If not a general question, check for products
        ...
        # If no products found, provide a more helpful response
